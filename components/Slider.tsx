...
const Slider = () => {
    ...
    return (
        <div className="h-[calc(700px-80px)] overflow-hidden py-7 bg-light-softBg dark:bg-dark-softBg">
            <div className="w-max h-full flex transition-all ease-in-out duration-1000" style={{ transform: `translateX(-${current * 100}vw)` }}>
                {slides.map(slide => (
                    <div className={`${slide.bg} w-screen h-full flex flex-col ap-16 xl:flex-row`} key={slide.id}>
                        ...
                        <div className=" xl:w-1/2 h-full relative">
                            <Image src={slide.img} alt="" fill sizes="100%" className="object-cover" />
                        </div>
                    </div>
                ))}
            </div>
            <div className="absolute m-auto left-1/2 flex gap-4 my-2">
